{{-- 
    Custom Pagination View với Tailwind CSS
    Sử dụng: {{ $items->links('pagination.tailwind') }}
    
    Chức năng:
    - Hiển thị phân trang với Tailwind CSS styling
    - Tự động giữ lại query parameters (như keyword search)
            ...
            <div>
                <p class="text-sm text-gray-700 leading-5">
                    @if ($paginator->firstItem())
                        Hiển thị <span class="font-medium">{{ $paginator->firstItem() }}</span>
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="Đi đến trang {{ $page }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
