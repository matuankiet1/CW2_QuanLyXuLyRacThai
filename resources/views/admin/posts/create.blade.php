...
@section('content')
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold mb-6">Thêm bài viết mới</h2>
        ...
        <form method="POST" action="{{ route('admin.posts.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="excerpt" rows="3" required
                    class="w-full border border-gray-300 p-2 rounded focus:outline-none focus:ring focus:ring-blue-200">{{ old('excerpt') }}</textarea>
                @error('excerpt')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="content" rows="6" required
                    class="w-full border border-gray-300 p-2 rounded focus:outline-none focus:ring focus:ring-blue-200">{{ old('content') }}</textarea>
                @error('content')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="post_categories"
                    ...
                    <option value="Kiến thức" {{ old('post_categories') == 'Kiến thức' ? 'selected' : '' }}>Kiến thức</option>
            ...
            <div class="mb-4">
                ...
                {{-- Ảnh preview --}}
                <div class="mt-3">
                    <img id="imagePreview" src="#" alt="Xem trước ảnh" class="hidden w-48 h-48 object-cover rounded border">
                </div>
