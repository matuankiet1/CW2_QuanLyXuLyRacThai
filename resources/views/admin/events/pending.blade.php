...
<table class="w-full table-auto bg-white rounded-lg shadow overflow-hidden">
    ...
    <tbody>
        ...
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2">{{ $participant->registered_at->format('d/m/Y H:i') }}</td>
            <td class="px-4 py-2">
                <form action="{{ route('admin.events.participants.confirm', [$event->id, $participant->user_id]) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">Xác nhận</button>
                </form>
            </td>
        </tr>
        @empty
